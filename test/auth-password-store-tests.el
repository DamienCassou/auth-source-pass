...
(auth-pass-deftest any-host-max-1 ()
                   '(("foo" ("port" . "foo-port") ("user" . "foo-user"))
                     ("bar"))
  (pcase (auth-source-search :host t :max 1)
    (`((:host "foo" :port "foo-port" :user "foo-user" :secret ,_)) t)
    (_ (ert-fail (auth-source-search :host t :max 1)))))

(auth-pass-deftest any-host-max-2 ()
                   '(("foo" ("port" . "foo-port") ("user" . "foo-user"))
                     ("bar" ("port" . "bar-port") ("user" . "bar-user")))
  (pcase (auth-source-search :host t :max 2)
    (`((:host "foo" :port "foo-port" :user "foo-user" :secret ,_)
       (:host "bar" :port "bar-port" :user "bar-user" :secret ,_)) t)
    (_ (ert-fail (auth-source-search :host t :max 2)))))

(auth-pass-deftest any-host-default-max-1 ()
                   '(("foo"  ("port" . "foo-port") ("user" . "foo-user"))
                     ("bar"))
  (pcase (auth-source-search :host t)
    (`((:host "foo" :port "foo-port" :user "foo-user" :secret ,_)) t)
    (_ (ert-fail (auth-source-search :host t)))))

(auth-pass-deftest any-host-max-0 ()
                   '(("foo" ("port" . "foo-port") ("user" . "foo-user"))
                     ("bar"))
  (pcase (auth-source-search :host t :max 0)
    (`t t)
    (_ (ert-fail (auth-source-search :host t :max 0)))))

(auth-pass-deftest no-parameter-default-max-1 ()
                   '(("foo" ("port" . "foo-port") ("user" . "foo-user"))
                     ("bar"))
  (pcase (auth-source-search)
    (`((:host "foo" :port "foo-port" :user "foo-user" :secret ,_)) t)
    (_ (ert-fail (auth-source-search)))))

(auth-pass-deftest specified-host-default-max-1 ()
                   '(("foo")
                     ("bar" ("port" . "bar-port") ("user" . "bar-user")))
  (pcase (auth-source-search :host "bar")
    (`((:host "bar" :port "bar-port" :user "bar-user" :secret ,_)) t)
    (_ (ert-fail (auth-source-search :host "bar")))))

(auth-pass-deftest specified-host-singleton-default-max-1 ()
                   '(("foo")
                     ("bar" ("port" . "bar-port") ("user" . "bar-user")))
  (pcase (auth-source-search :host '("bar"))
    (`((:host "bar" :port "bar-port" :user "bar-user" :secret ,_)) t)
...
(auth-pass-deftest specified-host-list-max-1 ()
                   '(("foo")
                     ("bar" ("port" . "bar-port") ("user" . "bar-user"))
                     ("baz"))
  (pcase (auth-source-search :host '("bar" "baz") :max 1)
    (`((:host "bar" :port "bar-port" :user "bar-user" :secret ,_)) t)
    (_ (ert-fail (auth-source-search :host '("bar" "baz") :max 1)))))

(auth-pass-deftest specified-host-list-max-2 ()
                   '(("foo")
                     ("bar" ("port" . "bar-port") ("user" . "bar-user"))
                     ("baz" ("port" . "baz-port") ("user" . "baz-user")))
  (pcase (auth-source-search :host '("bar" "baz") :max 2)
    (`((:host "bar" :port "bar-port" :user "bar-user" :secret ,_)
       (:host "baz" :port "baz-port" :user "baz-user" :secret ,_)) t)
    (_ (ert-fail (auth-source-search :host '("bar" "baz") :max 2)))))
